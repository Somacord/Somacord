...
- **Free tier has zero enforcement.** [pricing.md](../business/pricing.md#free)'s "limited RSVPs" for non-subscribers isn't backed by anything — RLS and the RSVP action currently let any signed-in user (with or without an active `memberships` row) RSVP without limit.
